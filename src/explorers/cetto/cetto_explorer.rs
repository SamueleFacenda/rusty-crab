...
use crate::explorers::cetto::communication::{OrchestratorLoggingReceiver, OrchestratorLoggingSender,
...
impl Bag {
    fn get_basic(&mut self, target_type: BasicResourceType) -> Option<BasicResource> {
        if let Some(index) = self.basic_resources.iter().position(|res| res.get_type() == target_type) {
    ...
    fn get_complex(&mut self, target_type: ComplexResourceType) -> Option<ComplexResource> {
        if let Some(index) = self.complex_resources.iter().position(|res| res.get_type() == target_type) {
...
impl CettoExplorer {
    fn handle_orchestrator_message(&mut self, msg: OrchestratorToExplorer) -> Result<(), String> {
        ...
        match msg {
            ...
            OrchestratorToExplorer::SupportedResourceRequest => {
                ...
                self.tx_planets[&self.current_planet_id]
                    .send(ExplorerToPlanet::SupportedResourceRequest { explorer_id: self.id })?;
                let planet_response = self
                    .rx_planet
                    .recv()
                    .map_err(|err| format!("Exception when waiting for planet response: {err}"))?;

                self.tx_orchestrator.send(ExplorerToOrchestrator::SupportedResourceResult {
                    explorer_id: self.id,
            ...
            OrchestratorToExplorer::SupportedCombinationRequest => {
                // Ask the current planet, wait for response, and then respond to the orchestrator
                self.tx_planets[&self.current_planet_id]
                    .send(ExplorerToPlanet::SupportedCombinationRequest { explorer_id: self.id })?;
                let planet_response = self
                    .rx_planet
                    .recv()
                    .map_err(|err| format!("Exception when waiting for planet response: {err}"))?;

                self.tx_orchestrator.send(ExplorerToOrchestrator::SupportedCombinationResult {
                    explorer_id: self.id,
            ...
            OrchestratorToExplorer::GenerateResourceRequest { to_generate } => {
                ...
                self.tx_orchestrator
                    .send(ExplorerToOrchestrator::GenerateResourceResponse { explorer_id: self.id, generated: Ok(()) })
            }
            OrchestratorToExplorer::CombineResourceRequest { to_generate } => {
                self.handle_combine_resource_request(to_generate)?;
                // Respond to Orchestrator
                self.tx_orchestrator
                    .send(ExplorerToOrchestrator::CombineResourceResponse { explorer_id: self.id, generated: Ok(()) })
            }
            OrchestratorToExplorer::BagContentRequest => {
                ...
                // Respond
                let content = BagContent { content: self.bag.generate_bag_content_hashmap() };
                self.tx_orchestrator
                    .send(ExplorerToOrchestrator::BagContentResponse { explorer_id: self.id, bag_content: content })
            }
    ...
    fn request_neighbors(&mut self) -> Result<(), String> {
        self.tx_orchestrator.send(ExplorerToOrchestrator::NeighborsRequest {
            explorer_id: self.id,
    ...
    fn move_explorer(&mut self, dst_planet_id: ID) -> Result<(), String> {
        self.tx_orchestrator.send(ExplorerToOrchestrator::TravelToPlanetRequest {
            explorer_id: self.id,
            current_planet_id: self.current_planet_id,
            dst_planet_id
        })?;
    ...
    fn exploit_current_planet(&mut self) -> Result<(), String> {
        ...
        if cells == 0 {
            return Ok(());
        } // Again, to possibly lower the number of comms
    ...
    fn has_ingredients(&self, typ: ComplexResourceType) -> bool {
        match typ {
            ...
            ComplexResourceType::Diamond => self.bag.has_basic(BasicResourceType::Carbon, 2),
            ...
            ComplexResourceType::Dolphin =>
                self.bag.has_complex(ComplexResourceType::Water, 1)
                    && self.bag.has_complex(ComplexResourceType::Life, 1),
            ComplexResourceType::AIPartner =>
                self.bag.has_complex(ComplexResourceType::Robot, 1)
                    && self.bag.has_complex(ComplexResourceType::Diamond, 1),
        }
    }
    ...
    fn get_available_cells(&self) -> Result<u32, String> {
        self.tx_planets[&self.current_planet_id]
            .send(ExplorerToPlanet::AvailableEnergyCellRequest { explorer_id: self.id })?;
        let cells = self
            .rx_planet
            .recv()
            .map_err(|err| format!("Exception when waiting for planet response: {err}"))?
    ...
    fn handle_move_planet(
        ...
        self.tx_orchestrator.send(ExplorerToOrchestrator::MovedToPlanetResult {
            explorer_id: self.id,
            planet_id: self.current_planet_id
        })
    }
    ...
    fn handle_combine_resource_request(&mut self, to_generate: ComplexResourceType) -> Result<(), String> {
        ...
        self.tx_planets[&self.current_planet_id]
            .send(ExplorerToPlanet::CombineResourceRequest { explorer_id: self.id, msg: comp_res_req })?;
    ...
    fn handle_generate_resource_request(&mut self, to_generate: BasicResourceType) -> Result<(), String> {
        self.tx_planets[&self.current_planet_id]
            .send(ExplorerToPlanet::GenerateResourceRequest { explorer_id: self.id, resource: to_generate })?;
        let planet_response =
            self.rx_planet.recv().map_err(|err| format!("Exception when waiting for planet response: {err}"))?;
