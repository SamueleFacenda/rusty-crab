...
use common_game::protocols::planet_explorer::{ExplorerToPlanet, PlanetToExplorer, PlanetToExplorerKind};
use common_game::utils::ID;
use crossbeam_channel::Sender;

use super::{PlanetLoggingReceiver, PlanetLoggingSender};
...
impl PlanetsCommunicator {
    pub fn new(
        ...
        PlanetsCommunicator { to_planets, planets_rx, explorer_id }
    }
    ...
    pub fn set_current_planet(&mut self, planet_id: ID) { self.planets_rx.set_other_id(planet_id); }
    ...
    pub fn generate_basic_resource(
        &self,
        planet_id: ID,
        resource: BasicResourceType
    ) -> Result<Option<BasicResource>, String> {
        Ok(self
            .req_ack(
                planet_id,
                ExplorerToPlanet::GenerateResourceRequest { explorer_id: self.explorer_id, resource },
    ...
    ) -> Result<Result<ComplexResource, (String, GenericResource, GenericResource)>, String> {
        Ok(self
            .req_ack(
                planet_id,
                ExplorerToPlanet::CombineResourceRequest { explorer_id: self.explorer_id, msg },
    ...
    fn recv_timeout(&self) -> Result<PlanetToExplorer, String> {
        ...
        self.planets_rx.recv_timeout(timeout).map_err(|e| format!("Error waiting for message from planet: {e}"))
    }
}
