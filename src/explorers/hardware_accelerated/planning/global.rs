...
const FINAL_RESOURCES: [ComplexResourceType; 2] = [
...
impl GlobalTask {
    pub fn new(resource: ComplexResourceType) -> Self {
        GlobalTask { resource }
    }
}
...
impl GlobalPlanner {
    pub fn plan_next_task(state: &ExplorerState) -> GlobalTask {
        let min_res = FINAL_RESOURCES.iter().map(|res_type| {
            let count = state.bag.res.get(&ResourceType::Complex(*res_type))
                .map(|v|v.len())
                .unwrap_or_default();
            (res_type, count)
