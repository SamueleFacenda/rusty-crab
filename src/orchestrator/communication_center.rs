...
use common_game::protocols::orchestrator_planet::{OrchestratorToPlanet, OrchestratorToPlanetKind, PlanetToOrchestrator, PlanetToOrchestratorKind};
use common_game::utils::ID;

use crate::orchestrator::{BagContent, ExplorerChannelDemultiplexer, ExplorerLoggingSender, PlanetChannelDemultiplexer, PlanetLoggingSender};
...
impl CommunicationCenter {
    ...
    pub fn send_to_planet(&mut self, planet_id: ID, msg: OrchestratorToPlanet) -> Result<(), String> {
        self.to_planets[&planet_id]
            .send(msg, planet_id)
            .map_err(|e| e.to_string())
    }
    
    pub fn send_to_explorer(&mut self, explorer_id: ID, msg: OrchestratorToExplorer) -> Result<(), String> {
        self.to_explorers[&explorer_id]
            .send(msg, explorer_id)
            .map_err(|e| e.to_string())
    }

    pub fn planet_syn_ack(&mut self, planet_id: ID, msg: OrchestratorToPlanet, expected: PlanetToOrchestratorKind) -> Result<PlanetToOrchestrator, String> {
        self.send_to_planet(planet_id, msg)?;
        self.planets_rx
            .recv_from(planet_id)
            .map(|res| {
                if PlanetToOrchestratorKind::from(&res) != expected {
                    Err(format!("Expected planet {planet_id} to respond with {expected:?}, but got {res:?}"))
                } else {
                    Ok(res)
                }
            })?
    ...
    pub fn explorer_syn_ack(&mut self, explorer_id: ID, msg: OrchestratorToExplorer, expected: ExplorerToOrchestratorKind) -> Result<ExplorerToOrchestrator<BagContent>, String> {
        self.send_to_explorer(explorer_id, msg)?;
        self.explorers_rx
            .recv_from(explorer_id)
            .map(|res| {
                if ExplorerToOrchestratorKind::from(&res) != expected {
                    Err(format!("Expected explorer {explorer_id} to respond with {expected:?}, but got {res:?}"))
                } else {
                    Ok(res)
                }
            })?
    ...
    pub fn recv_from_explorer(&mut self, explorer_id: ID) -> Result<ExplorerToOrchestrator<BagContent>, String> {
